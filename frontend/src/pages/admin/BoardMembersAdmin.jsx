...
const BoardMembersAdmin = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await contentAPI.deleteBoardMember(id);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Члены правления</h1>
        <button
          onClick={() => setEditModal({ ...emptyMember })}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="font-medium text-gray-900">
                  {member.last_name_ru} {member.first_name_ru} {member.patronymic_ru}
                </h3>
                {!member.is_active && (
              ...
              <p className="text-sm text-gray-500">{member.position_ru}</p>
              <p className="text-xs text-gray-400">{member.degree_ru}</p>
            </div>
            <div className="text-sm text-gray-400">#{member.order}</div>
            <div className="flex items-center gap-2">
              <button
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-4xl w-full p-6 my-8">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              {editModal.id ? 'Редактировать' : 'Добавить'} члена правления
            </h3>
            ...
            <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                    placeholder="Доктор мед. наук, профессор"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                    placeholder="Tibbiyot fanlari doktori"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                    placeholder="Doctor of Medical Sciences"
                  />
                </div>
              </div>
              ...
              {/* Bio - large textarea for extensive description */}
              <div className="border-t pt-4">
                <h4 className="text-md font-semibold text-gray-800 mb-3 flex items-center gap-2">
                  <svg className="w-5 h-5 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  Биография (подробное описание)
                </h4>
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    ...
                    <textarea
                      ...
                      rows={5}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      placeholder="Подробная биография: образование, карьерный путь, специализация, научные интересы..."
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Biografiya (UZ)</label>
                    <textarea
                      value={editModal.bio_uz || ''}
                      onChange={(e) => updateField('bio_uz', e.target.value)}
                      rows={5}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      placeholder="Batafsil biografiya..."
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Biography (EN)</label>
                    <textarea
                      value={editModal.bio_en || ''}
                      onChange={(e) => updateField('bio_en', e.target.value)}
                      rows={5}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      placeholder="Detailed biography: education, career path, specialization, research interests..."
                    />
                  </div>
                </div>
              </div>

              {/* Achievements - large textarea for awards, honors, publications */}
              <div className="border-t pt-4">
                <h4 className="text-md font-semibold text-gray-800 mb-3 flex items-center gap-2">
                  <svg className="w-5 h-5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
                  </svg>
                  Достижения, награды, регалии
                </h4>
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Достижения (RU)</label>
                    <textarea
                      value={editModal.achievements_ru || ''}
                      onChange={(e) => updateField('achievements_ru', e.target.value)}
                      rows={4}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      placeholder="Награды, почётные звания, членство в организациях, публикации, патенты..."
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Yutuqlar (UZ)</label>
                    <textarea
                      value={editModal.achievements_uz || ''}
                      onChange={(e) => updateField('achievements_uz', e.target.value)}
                      rows={4}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      placeholder="Mukofotlar, faxriy unvonlar, tashkilotlarga a'zolik..."
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Achievements (EN)</label>
                    <textarea
                      value={editModal.achievements_en || ''}
                      onChange={(e) => updateField('achievements_en', e.target.value)}
                      rows={4}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                      placeholder="Awards, honors, memberships, publications, patents..."
                    />
                  </div>
                </div>
              </div>
